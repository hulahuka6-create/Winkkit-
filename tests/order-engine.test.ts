...
const customer: User = { id: 7, openId: "customer-7", email: "customer@example.com", name: "Customer", loginMethod: "manus", role: "user", createdAt: new Date(), updatedAt: new Date(), lastSignedIn: new Date() };
...
describe("Winkkit order engine contracts", () => {
  it("rejects order creation for unauthenticated customers", async () => {
    ...
    await expect(caller.orders.create({ shopId: 1, items: [{ productId: 1, quantity: 1 }] })).rejects.toMatchObject({ code: "UNAUTHORIZED" });
  });

  it("rejects empty carts before any database call", async () => {
    const caller = appRouter.createCaller(context(customer));
    await expect(caller.orders.create({ shopId: 1, items: [] })).rejects.toMatchObject({ code: "BAD_REQUEST" });
  });
